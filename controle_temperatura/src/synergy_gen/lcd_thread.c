...
TX_QUEUE lcd_out_queue;
static uint8_t queue_memory_lcd_out_queue[500];
TX_QUEUE lcd_in_queue;
static uint8_t queue_memory_lcd_in_queue[200];
...
void lcd_thread_create(void)
{
    ...
    UINT err_lcd_out_queue;
    err_lcd_out_queue = tx_queue_create (&lcd_out_queue, (CHAR*) "LCD out Queue", 1, &queue_memory_lcd_out_queue,
                                         sizeof(queue_memory_lcd_out_queue));
    if (TX_SUCCESS != err_lcd_out_queue)
    {
        tx_startup_err_callback (&lcd_out_queue, 0);
    }
    UINT err_lcd_in_queue;
    err_lcd_in_queue = tx_queue_create (&lcd_in_queue, (CHAR*) "LCD in Queue", 2, &queue_memory_lcd_in_queue,
                                        sizeof(queue_memory_lcd_in_queue));
    if (TX_SUCCESS != err_lcd_in_queue)
    {
        tx_startup_err_callback (&lcd_in_queue, 0);
    }
